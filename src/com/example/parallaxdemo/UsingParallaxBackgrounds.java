...
/**
 * This activity is allowing us to apply a feeling of depth to our scene through
 * the use of the ParallaxBackground class. The ParallaxBackground class allows
 * us to create ParallaxEntities from Sprite objects, which when applied to the
 * scene with a defined parallax factor, will react to camera movement at a rate
 * dependent on its own factor. The final outcome is 3 different hill sprites
 * "scrolling" the screen at different speeds upon a moving camera, giving the
 * sense that some sprites appear closer or farther than others. Additionally,
 * this activity registers user touch events for the movement of the camera.
 * 
 * Original Source:
 * https://github.com/thaind/Android/blob/master/Demos/Andengine
 * /8987OS_Code_Bundle/8987OS_03_Code/UsingParallaxBackgrounds.java
 * 
 * Example found on github, and included on the book:
 * "AndEngine for Android Game Development Cookbook", by Jayme Schroeder and
 * Brian Broyles, 2013 Packt Publishing.
 * 
 */
public class UsingParallaxBackgrounds extends BaseGameActivity {
	...
	public EngineOptions onCreateEngineOptions() {
		...
		mCamera = new Camera(0, 0, WIDTH, HEIGHT) {
			...
			/*
			 * Boolean value which will determine whether to increase or
			...
			public void onUpdate(float pSecondsElapsed) {
				...
				if (incrementX) {
					...
					/*
					 * If the new offset coordinate is greater than the max X
					 * limit
					 */
	...
	public void onCreateResources(OnCreateResourcesCallback pOnCreateResourcesCallback) {
		 ...
		 */
		BuildableBitmapTextureAtlas bitmapTextureAtlas = new BuildableBitmapTextureAtlas(mEngine.getTextureManager(),
				800, 150);

		/* Create the mHillTextureRegion, passing in the hill.png image */
		mHillTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(bitmapTextureAtlas, getAssets(),
				"hill.png");

		/*
		 * Build the texture atlas. Since we're loading only a single image into
		 * the texture atlas, we need not worry about padding or spacing values
		 */
		try {
			bitmapTextureAtlas.build(new BlackPawnTextureAtlasBuilder<IBitmapTextureAtlasSource, BitmapTextureAtlas>(0,
					0, 0));
	...
	public void onCreateScene(OnCreateSceneCallback pOnCreateSceneCallback) {
		...
		/*
		 * Obtain the mHillTextureRegion image height for the purpose of placing
		 * the Sprites at different heights on the Scene
		 */
		final float textureHeight = mHillTextureRegion.getHeight();

		/*
		 * Create the hill which will appear to be the furthest into the
		 * distance. This Sprite will be placed higher than the rest in order to
		 * retain visibility of it
		 */
		Sprite hillFurthest = new Sprite(WIDTH * 0.5f, textureHeight * 0.5f + 100, mHillTextureRegion,
		 ...
		 */
		Sprite hillMid = new Sprite(WIDTH * 0.5f, textureHeight * 0.5f + 50, mHillTextureRegion,
				mEngine.getVertexBufferObjectManager());

		/*
		 * Create the closest hill which will not be obstructed by any other
		 * hill Sprites. This Sprite will be placed at the bottom of the Scene
		 * since nothing will be covering its view
		 */
